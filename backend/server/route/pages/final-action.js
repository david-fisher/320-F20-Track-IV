...
router
  .get("/", headers.areHeadersValid, auth.isAuthenticated, async (req, res) => {
    ...
    try {
      const finalActionPageGroup = await db.pageGroup.finalAction.getFinalActionPageGroup(
        scenarioID
      );
      res.status(httpStatusCode.success.OK);
      res.json({
        success: true,
        ...finalActionPageGroup,
      });
    } catch (error) {
  ...
  .post(
    ...
    async (req, res) => {
      ...
      try {
        const finalActionPageGroup = await db.pageGroup.finalAction.createFinalActionPageGroup(
          scenarioID,
          bodyText,
          prompts,
          content,
