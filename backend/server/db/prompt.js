...
exports.getPrompt = async function (pageID, promptNum) {
  ...
  const { rows } = await db.query(query, [pageID, promptNum]);
...
exports.createPrompt = async function (pageID, prompt) {
  ...
  const { rows } = await db.query(query, [pageID, prompt]);
  return rows[0];
};

exports.updatePrompt = async function (pageID, promptNum, text) {
  const query = "UPDATE prompt SET body_text = $3 WHERE page_id = $1 and prompt_num = $2";
  const { rows } = await db.query(query, [pageID, promptNum, text]);
  return rows[0];
};
